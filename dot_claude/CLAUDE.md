...
- Never add Claude/AI attribution anywhere: no `🤖 Generated with Claude Code`, no `Co-Authored-By: Claude`, no "generated by" footers — in commit messages, PR titles/descriptions, issue comments, or code comments.
- `includeCoAuthoredBy` is already `false`, so the harness adds nothing automatically; the only way this leaks in is by typing it manually, so don't. PR bodies and commits end on substantive content.
